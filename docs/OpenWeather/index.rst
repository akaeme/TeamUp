.. OpenWeather documentation master file, created by
   sphinx-quickstart on Wed Jan  3 18:31:06 2018.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to ``OpenWeather`` Service's documentation!
===================================================

Overview
--------
``OpenWeather`` API is a service that provides a 10 day weather forecast per location and a precise temperature for a given day and
geographic coordinates. This service communicates with the external API, OpenWeather.
...
-----------------------
``OpenWeather requests`` can assume multiple types according to the desired operation.
Only ``GET/POST/DELETE`` HTTP request methods are allowed.
The request data is appended on the body for ``POST`` requests and on arguments for ``DELETE`` and ``GET``. The request and response body have JSON formatting.
...
--------------------------
``OpenWeather Manager`` service is authenticated over an external identify that must be trusted. The service need to be registered on the authentication central server and need to have the credentials. To be authenticated you need 2 steps/requests:
...
The service export 2 interfaces to handle all the communications:
    ...
    - Authorization Managment
        An interface that allows to create multiple apps on the service. To accomplish this the client must be authenticated and indicate one url to where the grant will be redirected and the desired scopes, to get different views from the service. The service will respond with a `client id` and a `client secret`::
            ...
            http://127.0.0.1:5009/Opw/v1.1/authorization_managment/
...
Resources
---------
This service had only one resource, the ``Opw``. This resource methods according to its context.
...
- ``OpenW`` - Error fetching the prevision.
...
- ``Opw Resource``
    - ``POST``
        requests.post('http://127.0.0.1:5009/Opw/', json={'lat': '40.63036952784689', 'long':'-8.657569885253906', 'day': 22, 'hour': 15,'access_token': 'helloworld'})

    ::

         curl -H "Content-Type: application/json" -X POST -d '{'lat': '40.63036952784689', 'long':'-8.657569885253906', 'day': 22, 'hour': 15,'access_token': 'helloworld'}' http://127.0.0.1:5009/Opw/
