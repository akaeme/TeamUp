...
logger = logging.getLogger('UndergroundWeather Logger')
...
fileHandler = logging.FileHandler("{}.log".format('UndergroundWeather'))
fileHandler.setFormatter(formatter)
logger.addHandler(fileHandler)


app = Flask('UndergroundWeather')
app.logger.addHandler(ch)
app.secret_key = 'developement'
app.config.update({'SQLALCHEMY_DATABASE_URI': 'sqlite:///oauth_undergroundweather.sqlite'})
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db = SQLAlchemy(app)
# as provider
oauth = OAuth2Provider(app)
api = Api(app)
under_key = os.environ['UNDER_KEY']
JWT = None
...
class Hourly10days(Resource):
    ...
    def post(self, app_name):
        ...
        json_hour = data['hour']
        try:
            under = requests.get('http://api.wunderground.com/api/'+under_key+'/hourly10day/q/'+lat+','+lng+'.json')
        except requests.exceptions.RequestException as e:
            logger.exception(e)
            return error_message('Underground', 'Internal error', 500)

        response = under.json()['hourly_forecast']

        for i in range(len(response)):
            condition = response[i]['condition']
            day = response[i]['FCTTIME']['mday']
            hour = response[i]['FCTTIME']['hour']
            temp = response[i]['temp']['metric']
            if json_day == day and json_hour == hour:
                logger.info('HTTP POST Weather - successfully processed')
                logger.info('{}'.format(temp))
                return {'hour': hour,'temp': "%.1f" % float(temp), 'condition':condition, 'day':day}
        return []
...
api.add_resource(Hourly10days, '/Underground/')
api.add_resource(Internal, '/Underground/v1.1/internal/')
api.add_resource(Authorization, '/Underground/v1.1/authorization/')
api.add_resource(AuthorizationManagment, '/Underground/v1.1/authorization_managment/')
...
if __name__ == '__main__':
    ...
    user = User.query.filter_by(service_name='UndergroundWeather').first()
    if not user:
        user = User(id=1, service_name='UndergroundWeather')
        db.session.add(user)
        db.session.commit()
    app.run(port=5010, host='0.0.0.0', threaded=True)
