...
logger = logging.getLogger('PushNotification Logger')
...
fileHandler = logging.FileHandler("{}.log".format('PushNotificationService'))
fileHandler.setFormatter(formatter)
logger.addHandler(fileHandler)

app = Flask('PushNotificationService')
app.logger.addHandler(ch)
app.secret_key = 'development'
app.config.update({'SQLALCHEMY_DATABASE_URI': 'sqlite:///oauth_push_notifications.sqlite'})
...
class SendNotification(Resource):
    ...
    @require_oauth('basic')
    def post(app_name):
        push_service = FCMNotification(api_key=PUSHCONF['API_KEY'])

        parser = reqparse.RequestParser()
        parser.add_argument('topic', type=str, location='json', required=True, help='queue topic cannot be blank')
        parser.add_argument('message', type=str, location='json', required=True, help='message cannot be blank')
        parser.add_argument('access_token', type=str, location='json', required=True)
        args = parser.parse_args(strict=True)

        push_service.notify_topic_subscribers(topic_name='event_' + args['topic'], message_body=args['message'])
...
api.add_resource(SendNotification, '/pushNotification/v1.0/sendNotification/')
api.add_resource(Internal, '/pushNotification/v1.0/internal/')
api.add_resource(Authorization, '/pushNotification/v1.0/authorization/')
api.add_resource(AuthorizationManagment, '/pushNotification/v1.0/authorization_managment/')
...
if __name__ == '__main__':
    ...
    user = User.query.filter_by(service_name='PushNotificationService').first()
    if not user:
        user = User(id=1, service_name='PushNotificationService')
        db.session.add(user)
        db.session.commit()
    PUSHCONF = SERVICES['PUSH']
    app.run(debug=False, host='0.0.0.0', port=5017, threaded=True)
